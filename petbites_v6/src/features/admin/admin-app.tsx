...
const definitions: Record<AdminEntity, EntityDefinition> = {
  birds: {
    ...
    fields: [
      ...
      { key: "name", label: "Nama — Indonesia", type: "text", required: true },
      {
        key: "name_en",
        label: "Name — English",
        type: "text",
        help: "Wajib diisi sebelum Published. Bisa dibuat lewat AI lalu direview.",
      },
      { key: "emoji", label: "Emoji cadangan", type: "text" },
      { key: "scientific_name", label: "Nama ilmiah", type: "text", required: true },
      { key: "description", label: "Deskripsi — Indonesia", type: "textarea", required: true },
      {
        key: "description_en",
        label: "Description — English",
        type: "textarea",
        help: "Wajib diisi sebelum Published.",
      },
  ...
  bird_foods: {
    ...
    fields: [
      ...
      { key: "name", label: "Nama pakan — Indonesia", type: "text", required: true },
      { key: "name_en", label: "Food name — English", type: "text" },
      {
        key: "category",
        label: "Kategori",
        type: "select",
      ...
      { key: "benefits", label: "Manfaat Indonesia (satu per baris)", type: "lines" },
      {
        key: "benefits_en",
        label: "English benefits (urutan harus sama)",
        type: "lines",
      },
      { key: "note", label: "Catatan — Indonesia", type: "textarea" },
      { key: "note_en", label: "Note — English", type: "textarea" },
  ...
  toxic_entries: {
    ...
    fields: [
      ...
      { key: "name", label: "Nama bahan — Indonesia", type: "text", required: true },
      { key: "name_en", label: "Ingredient name — English", type: "text" },
      {
        key: "status",
      ...
      { key: "explanation", label: "Penjelasan — Indonesia", type: "textarea", required: true },
      { key: "explanation_en", label: "Explanation — English", type: "textarea" },
      { key: "sort_order", label: "Urutan", type: "number" },
      { key: "content_status", label: "Status konten", type: "select", options: STATUS_OPTIONS },
      { key: "review_status", label: "Status review", type: "select", options: REVIEW_OPTIONS },
      { key: "source_urls", label: "Sumber (satu URL per baris)", type: "lines" },
  ...
  portion_rules: {
    ...
    fields: [
      ...
      { key: "teaspoon", label: "Perkiraan sendok — Indonesia", type: "text", required: true },
      { key: "teaspoon_en", label: "Estimated volume — English", type: "text" },
      { key: "morning", label: "Jadwal pagi — Indonesia", type: "text", required: true },
      { key: "morning_en", label: "Morning schedule — English", type: "text" },
      { key: "evening", label: "Jadwal sore — Indonesia", type: "text", required: true },
      { key: "evening_en", label: "Evening schedule — English", type: "text" },
      { key: "sort_order", label: "Urutan", type: "number" },
      { key: "content_status", label: "Status konten", type: "select", options: STATUS_OPTIONS },
      { key: "review_status", label: "Status review", type: "select", options: REVIEW_OPTIONS },
      { key: "source_urls", label: "Sumber (satu URL per baris)", type: "lines" },
  ...
  recipes: {
    ...
    fields: [
      { key: "id", label: "ID / slug", type: "text", required: true },
      { key: "bird_id", label: "Burung", type: "select", required: true },
      { key: "title", label: "Judul resep — Indonesia", type: "text", required: true },
      { key: "title_en", label: "Recipe title — English", type: "text" },
      { key: "purpose", label: "Tujuan — Indonesia", type: "textarea", required: true },
      { key: "purpose_en", label: "Purpose — English", type: "textarea" },
      {
        key: "ingredients",
        label: "Bahan Indonesia (satu per baris)",
        type: "lines",
        required: true,
      },
      {
        key: "ingredients_en",
        label: "English ingredients (urutan harus sama)",
        type: "lines",
      },
      { key: "steps", label: "Langkah Indonesia (satu per baris)", type: "lines", required: true },
      { key: "steps_en", label: "English steps (urutan harus sama)", type: "lines" },
      { key: "sort_order", label: "Urutan", type: "number" },
      { key: "content_status", label: "Status konten", type: "select", options: STATUS_OPTIONS },
      { key: "review_status", label: "Status review", type: "select", options: REVIEW_OPTIONS },
      { key: "source_urls", label: "Sumber (satu URL per baris)", type: "lines" },
...
export function AdminApp() {
  ...
  useEffect(() => {
    ...
    setProfileError("");
    setData(null);
    setProfileReady(!userId);

    if (!userId) return () => undefined;

    void fetchAdminProfile(userId)
      .then((nextProfile) => {
        if (active) setProfile(nextProfile);
      })
      .catch((caught) => {
        if (active) setProfileError(messageFrom(caught));
      })
      .finally(() => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (requestId === loadRequestRef.current) setError(messageFrom(caught));
    } finally {
  ...
  const operationBusy = saving || refreshing || uploading;
  ...
  function selectRecord(record: AdminRecord) {
    if (operationBusy || !confirmDiscardChanges()) return;
    cancelPendingAi();
  ...
  function confirmDiscardChanges() {
    return (
      !dirty ||
      window.confirm("Ada perubahan pada form yang belum disimpan. Buang perubahan tersebut?")
    );
  }

  function changeEntity(nextEntity: AdminEntity) {
    if (operationBusy || nextEntity === entity || !confirmDiscardChanges()) return;
    cancelPendingAi();
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      await saveAdminRecord(entity, payload, { isNew });
  ...
  async function refreshCurrentRecord() {
    if (!profile || operationBusy) return;
  ...
  async function remove() {
    if (!canEdit || operationBusy || isNew || !draft.id) return;
    cancelPendingAi();
    if (
      !window.confirm(
  ...
  async function upload(file: File | null) {
    if (!canEdit || operationBusy || !file || entity !== "birds") return;
    cancelPendingAi();
    setUploading(true);
    setError("");
    try {
  ...
  async function askAi() {
    if (operationBusy || aiBusy) return;
    ...
    try {
      ...
      setEditorFeedback({
        ...
        message:
          aiAction === "translate"
            ? "Draft English berhasil dibuat. Periksa lalu terapkan ke kolom English."
            : aiAction === "review"
      ...
      if (requestId === aiRequestRef.current) setAiBusy(false);
    }
  }
  ...
  function applyAiSuggestion() {
    if (
      !canEdit ||
      !aiSuggestion ||
      (aiAction !== "draft" && aiAction !== "improve" && aiAction !== "translate")
    )
      return;

    const sanitized = sanitizeAiSuggestion(entity, aiAction, aiSuggestion);
  ...
  function createBirdDraftFromRequest() {
    if (operationBusy || entity !== "bird_requests" || isNew) return;
    if (!confirmDiscardChanges()) return;
  ...
  if (!profileReady) return <AdminLoading label="Memeriksa akses CMS…" />;
  if (profileError) {
    return (
      <AdminMessage
        title="Gagal memeriksa akses CMS"
        copy={`${profileError} Periksa koneksi dan pastikan migration CMS sudah dijalankan.`}
  ...
  if (!profile) {
    return (
      <AdminMessage
        ...
        copy="Login berhasil, tetapi user ini belum terdaftar di tabel admin_users. Ikuti bagian bootstrap admin di SUPABASE_SETUP.md."
        onSignOut={() => void handleSignOut()}
      />
    );
  }
        ...
        <section className="admin-workspace">
          <aside className="admin-list-panel">
            <div className="admin-panel-heading">
              ...
              {entity !== "bird_requests" && (
                <Button onClick={newRecord} disabled={operationBusy} className="gap-2">
          ...
          <section className="admin-editor-panel">
            ...
            <form onSubmit={save} className="admin-form">
              <div className="admin-fields">
                {definition.fields.map((field) => (
                  <AdminField
                    key={field.key}
                    field={field}
                    required={field.required && !(isNew && field.key === "id")}
                    value={draft[field.key]}
                    birds={data?.birds ?? []}
                    disabled={!canEdit || operationBusy || (!isNew && field.key === "id")}
                ...
                <div className="admin-media-box">
                  ...
                  <label className="admin-upload-button">
                    ...
                    <input
                      ...
                      disabled={operationBusy || !canEdit}
              ...
              {entity === "bird_requests" && !isNew && (
                <div className="admin-request-actions">
                  <Button
                    ...
                    variant="outline"
                    className="gap-2"
                    disabled={!canEdit || operationBusy}
              ...
              <div className="admin-form-actions">
                ...
                <Button type="submit" disabled={operationBusy || !canEdit} className="gap-2">
                  {saving ? (
                    <LoaderCircle className="h-4 w-4 animate-spin" />
                  ) : (
  ...
  return (
    <label className="admin-field">
      ...
      <Input
        ...
        min={field.key === "grams" ? "0.01" : field.type === "number" ? "0" : undefined}
        required={required}
        disabled={disabled}
    ...
    <section className="admin-ai-box">
      ...
      <div className="admin-ai-controls">
        <select
          ...
          onChange={(event) => onAction(event.target.value as AiAction)}
        >
        ...
        />
        <Button type="button" onClick={onAsk} disabled={disabled || busy} className="gap-2">
          {busy ? (
            <LoaderCircle className="h-4 w-4 animate-spin" />
          ) : (
      ...
      {(text || suggestion) && (
        <div className="admin-ai-result">
          ...
          <div className="admin-ai-result-actions">
            {suggestion && canApply && (
              <Button type="button" variant="outline" onClick={onApply} className="gap-2">
                ...
                {action === "translate" ? "Terapkan ke kolom English" : "Terapkan ke draft"}
              </Button>
            )}
          </div>
          {action === "review" && (
          ...
          {action === "translate" && (
            <p className="admin-ai-apply-notice">
              Hasil AI hanya mengisi draft kolom English. Admin tetap harus memeriksa lalu menekan
              Simpan.
            </p>
          )}
          {applyNotice && (
...
function aiActionHelp(action: AiAction) {
  switch (action) {
    ...
    case "translate":
      return "Membuat draft kolom English dari isi Indonesia. Periksa hasilnya sebelum diterapkan dan disimpan.";
    case "review":
...
const AI_TRANSLATION_FIELDS: Record<AdminEntity, ReadonlySet<string>> = {
  birds: new Set(["name_en", "description_en"]),
  bird_foods: new Set(["name_en", "benefits_en", "note_en"]),
  toxic_entries: new Set(["name_en", "explanation_en"]),
  portion_rules: new Set(["teaspoon_en", "morning_en", "evening_en"]),
  recipes: new Set(["title_en", "purpose_en", "ingredients_en", "steps_en"]),
...
function sanitizeAiSuggestion(
  ...
  const allowed =
    action === "translate" ? AI_TRANSLATION_FIELDS[entity] : AI_EDITABLE_FIELDS[entity];
  ...
  for (const [key, value] of Object.entries(suggestion)) {
    ...
    if (key === "grams") {
      ...
      if (Number.isFinite(grams) && grams > 0) {
        result[key] = Math.round((grams + Number.EPSILON) * 100) / 100;
      }
      continue;
    }
...
function recordToDraft(record: AdminRecord): AdminRecord {
  ...
  if ("benefits_en" in record) {
    draft.benefits_en = Array.isArray(record.benefits_en) ? record.benefits_en : [];
  }
  if ("ingredients" in record) {
    draft.ingredients = Array.isArray(record.ingredients) ? record.ingredients : [];
  }
  if ("ingredients_en" in record) {
    draft.ingredients_en = Array.isArray(record.ingredients_en) ? record.ingredients_en : [];
  }
  if ("steps" in record) {
    draft.steps = Array.isArray(record.steps) ? record.steps : [];
  }
  if ("steps_en" in record) {
    draft.steps_en = Array.isArray(record.steps_en) ? record.steps_en : [];
  }
...
function prepareRecord(entity: AdminEntity, draft: AdminRecord, requireId = true): AdminRecord {
  const record = { ...draft };
  normalizeDefinedTextFields(entity, record);
  ...
  if (entity === "bird_requests") {
    ...
    if (requireId) {
      ...
      validateTextLength(record.bird_name, "Nama burung", 2, 120);
      validateTextLength(record.reason, "Kebutuhan pengguna", 10, 1200);
      validateOptionalTextLength(record.local_name, "Nama lokal", 120);
      validateOptionalTextLength(record.scientific_name, "Nama ilmiah", 160);
      validateOptionalTextLength(record.contact, "Kontak", 180);
      validateOptionalTextLength(record.admin_notes, "Catatan admin", 3000);
    }
    return record;
  }

  const label =
    entity === "portion_rules"
      ? [record.size, record.condition].filter(Boolean).join("-")
  ...
  if (requireId) {
    ...
    if (!id || id !== slugify(id)) {
      throw new Error("ID/slug hanya boleh memakai huruf kecil, angka, dan tanda hubung.");
    }
    record.id = id;
  }
  ...
  if (record.note_en === "") record.note_en = null;
  if (record.image_url != null) validateImageUrl(record.image_url);

  record.sort_order = parseAdminNumber(record.sort_order, "Urutan", {
    defaultValue: 0,
    integer: true,
    minimum: 0,
  });

  if (entity === "portion_rules") {
    const hasGramValue = record.grams !== "" && record.grams !== null && record.grams !== undefined;
    if (requireId || hasGramValue) {
      const grams = parseAdminNumber(record.grams, "Gram per hari", { minimumExclusive: 0 });
      record.grams = Math.round((grams + Number.EPSILON) * 100) / 100;
    }
  }

  record.source_urls = normalizeStringArray(record.source_urls);
  validateSourceUrls(record.source_urls);
  if (entity === "bird_foods") {
    record.benefits = normalizeStringArray(record.benefits);
    record.benefits_en = normalizeStringArray(record.benefits_en);
  }
  if (entity === "recipes") {
    record.ingredients = normalizeStringArray(record.ingredients);
    record.ingredients_en = normalizeStringArray(record.ingredients_en);
    record.steps = normalizeStringArray(record.steps);
    record.steps_en = normalizeStringArray(record.steps_en);
  }

  if (requireId) {
    validateRequiredFields(entity, record);
    validateEnglishBeforePublish(entity, record);
  }
  return record;
}

function normalizeDefinedTextFields(entity: AdminEntity, record: AdminRecord) {
  for (const field of definitions[entity].fields) {
    if (field.type !== "text" && field.type !== "textarea" && field.type !== "select") continue;
    const value = record[field.key];
    if (typeof value === "string") record[field.key] = value.trim();
  }
}

function validateEnglishBeforePublish(entity: AdminEntity, record: AdminRecord) {
  if (record.content_status !== "published") return;

  const requireEnglishText = (key: string, label: string) => {
    const value = record[key];
    if (typeof value !== "string" || !value.trim()) {
      throw new Error(`${label} wajib diisi sebelum status Published.`);
    }
  };

  if (entity === "birds") {
    requireEnglishText("name_en", "Name — English");
    requireEnglishText("description_en", "Description — English");
    return;
  }

  if (entity === "bird_foods") {
    requireEnglishText("name_en", "Food name — English");
    const benefits = normalizeStringArray(record.benefits);
    const benefitsEn = normalizeStringArray(record.benefits_en);
    if (benefits.length !== benefitsEn.length) {
      throw new Error("Jumlah manfaat Indonesia dan English harus sama sebelum status Published.");
    }
    if (record.note && !String(record.note_en ?? "").trim()) {
      throw new Error("Note — English wajib diisi karena catatan Indonesia tidak kosong.");
    }
    return;
  }

  if (entity === "toxic_entries") {
    requireEnglishText("name_en", "Ingredient name — English");
    requireEnglishText("explanation_en", "Explanation — English");
    return;
  }

  if (entity === "portion_rules") {
    requireEnglishText("teaspoon_en", "Estimated volume — English");
    requireEnglishText("morning_en", "Morning schedule — English");
    requireEnglishText("evening_en", "Evening schedule — English");
    return;
  }

  if (entity === "recipes") {
    requireEnglishText("title_en", "Recipe title — English");
    requireEnglishText("purpose_en", "Purpose — English");
    const ingredients = normalizeStringArray(record.ingredients);
    const ingredientsEn = normalizeStringArray(record.ingredients_en);
    const steps = normalizeStringArray(record.steps);
    const stepsEn = normalizeStringArray(record.steps_en);
    if (ingredients.length !== ingredientsEn.length) {
      throw new Error("Jumlah bahan Indonesia dan English harus sama sebelum status Published.");
    }
    if (steps.length !== stepsEn.length) {
      throw new Error("Jumlah langkah Indonesia dan English harus sama sebelum status Published.");
    }
  }
}

function validateRequiredFields(entity: AdminEntity, record: AdminRecord) {
  for (const field of definitions[entity].fields) {
    if (!field.required) continue;
    const value = record[field.key];
    const emptyArray = Array.isArray(value) && value.length === 0;
    const emptyString = typeof value === "string" && value.trim() === "";
    if (value === null || value === undefined || emptyArray || emptyString) {
      throw new Error(`${field.label} wajib diisi.`);
    }
  }
}

function validateTextLength(value: unknown, label: string, minimum: number, maximum: number) {
  const length = String(value ?? "").length;
  if (length < minimum || length > maximum) {
    throw new Error(`${label} harus ${minimum} sampai ${maximum} karakter.`);
  }
}

function validateOptionalTextLength(value: unknown, label: string, maximum: number) {
  if (value === null || value === undefined || value === "") return;
  if (String(value).length > maximum) throw new Error(`${label} maksimal ${maximum} karakter.`);
}
...
function validateImageUrl(value: unknown) {
  ...
  assertHttpUrl(url, "URL gambar");
}

function validateSourceUrls(value: unknown) {
  const urls = Array.isArray(value) ? value.map(String) : [];
  if (urls.length > 20) throw new Error("Sumber maksimal 20 URL.");
  for (const url of urls) assertHttpUrl(url, "URL sumber");
}
...
function assertHttpUrl(value: string, label: string) {
  ...
  try {
    ...
    throw new Error(`${label} harus berupa alamat http:// atau https:// yang valid.`);
  }
}
...
function parseAdminNumber(
  ...
  if (!Number.isFinite(number)) throw new Error(`${label} harus berupa angka yang valid.`);
  if (options.integer && !Number.isInteger(number))
    throw new Error(`${label} harus bilangan bulat.`);
  if (options.minimum !== undefined && number < options.minimum) {
    throw new Error(`${label} minimal ${options.minimum}.`);
  }
  if (options.minimumExclusive !== undefined && number <= options.minimumExclusive) {
    throw new Error(`${label} harus lebih besar dari ${options.minimumExclusive}.`);
  }
