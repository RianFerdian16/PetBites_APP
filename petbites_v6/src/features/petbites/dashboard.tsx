...
export function BirdDashboard({
  ...
  const localizedBirdName = birdName(bird.name, bird.nameEn);
    ...
    <div className="dashboard-page">
      ...
      <section className="bird-profile">
        ...
        <div className="bird-profile__copy">
          ...
          <p className="bird-profile__description">
            {birdDescription(bird.description, bird.descriptionEn)}
          </p>
        </div>
...
function FoodFinder({ bird }: { bird: Bird }) {
  ...
  const filteredFoods = useMemo(() => {
    ...
    return bird.foods.filter((food) => {
      ...
      const localizedSearchText = `${contentText(food.name, food.nameEn)} ${food.benefits
        .map((benefit, index) => contentText(benefit, food.benefitsEn[index]))
        .join(" ")} ${food.note ? contentText(food.note, food.noteEn) : ""}`;
      const matchesQuery =
        !normalizedQuery ||
            ...
            <article className="food-card" key={food.id}>
              <div className="food-card__header">
                ...
                <div>
                  ...
                  <h3>{contentText(food.name, food.nameEn)}</h3>
                </div>
              </div>
                ...
                <ul className="benefit-list">
                  {food.benefits.map((benefit, benefitIndex) => (
                    <li key={`${food.id}-${benefitIndex}`}>
                      ...
                      {contentText(benefit, food.benefitsEn[benefitIndex])}
                    </li>
                  ))}
                </ul>
              )}
              {food.note && (
                <p className="food-card__note">{contentText(food.note, food.noteEn)}</p>
              )}
            </article>
          ))}
        </div>
...
function ToxicChecker({ bird }: { bird: Bird }) {
  ...
  const exactMatch = useMemo(() => {
    ...
    return (
      bird.toxic.find((entry) =>
        [entry.name, contentText(entry.name, entry.nameEn)]
          .map(normalize)
          .filter(Boolean)
  ...
  const suggestions = useMemo(() => {
    ...
    return bird.toxic
      .filter((entry) =>
        normalize(`${entry.name} ${contentText(entry.name, entry.nameEn)}`).includes(
          normalizedQuery,
        ),
      )
      .slice(0, 6);
        ...
        <div className="suggestion-row" aria-label={t("toxic.suggestions")}>
          {suggestions.map((entry) => (
            <button
              key={entry.id}
              type="button"
              onClick={() => setQuery(contentText(entry.name, entry.nameEn))}
            >
              {contentText(entry.name, entry.nameEn)}
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="avoid-list__grid">
          {toxicEntries.map((entry) => (
            <article key={entry.id}>
              ...
              <div>
                <h4>{contentText(entry.name, entry.nameEn)}</h4>
                <p>{contentText(entry.explanation, entry.explanationEn)}</p>
              </div>
            </article>
          ))}
        </div>
    ...
    <article className={`safety-result ${state.className}`}>
      ...
      <div>
        ...
        <div className="safety-result__title-row">
          <h3>{contentText(entry.name, entry.nameEn)}</h3>
          <Badge>{state.label}</Badge>
        </div>
        <p className="safety-result__summary">{state.description}</p>
        <p className="safety-result__detail">
          {contentText(entry.explanation, entry.explanationEn)}
        </p>
      </div>
    </article>
  );
        ...
        <div className="portion-result" aria-live="polite">
          ...
          <div className="portion-result__numbers">
            ...
            <div>
              ...
              <strong className="portion-result__teaspoon">
                {contentText(result.teaspoon, result.teaspoonEn)}
              </strong>
            </div>
          </div>
          ...
          <div className="feeding-timeline">
            <article>
              ...
              <div>
                ...
                <strong>{contentText(result.morning, result.morningEn)}</strong>
              </div>
            </article>
            ...
            <article>
              ...
              <div>
                ...
                <strong>{contentText(result.evening, result.eveningEn)}</strong>
              </div>
            </article>
          </div>
        </div>
        ...
        <details className="recipe-card" key={recipe.id} open={index === 0}>
          <summary>
            ...
            <span className="recipe-card__heading">
              <strong>{contentText(recipe.title, recipe.titleEn)}</strong>
              <small>{contentText(recipe.purpose, recipe.purposeEn)}</small>
            </span>
              ...
              <ul>
                {recipe.ingredients.map((ingredient, ingredientIndex) => (
                  <li key={`${recipe.id}-ingredient-${ingredientIndex}`}>
                    ...
                    {contentText(ingredient, recipe.ingredientsEn[ingredientIndex])}
                  </li>
                ))}
              </ul>
            </div>
              ...
              <ol>
                {recipe.steps.map((step, stepIndex) => (
                  <li key={`${recipe.id}-${stepIndex}`}>
                    ...
                    {contentText(step, recipe.stepsEn[stepIndex])}
                  </li>
                ))}
              </ol>
            </div>
