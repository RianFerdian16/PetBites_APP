...
## 3. Setup Database, Request Pengguna, dan CMS Admin

Untuk project Supabase baru, jalankan file berikut lewat **SQL Editor** sesuai urutan:
...
File pertama membuat tabel konten publik dan memasukkan data awal. File kedua menambahkan:

- Status `draft`, `review`, `published`, dan `archived`.
- Tabel admin dan role `owner`, `editor`, serta `reviewer`.
- Request burung dari pengguna.
- RLS untuk CMS dan konten publik.
- Audit log AI.
- Bucket upload gambar `bird-media`.

Setelah itu:

1. Buat user admin melalui **Authentication > Users**.
2. Ubah `EMAIL_ADMIN_KAMU` pada `supabase/bootstrap_admin.sql`.
3. Jalankan `supabase/bootstrap_admin.sql` di SQL Editor.
4. Pastikan query terakhir menampilkan akun dengan role `owner`.

Tanpa `supabase/cms_ai_request_setup.sql` dan `supabase/bootstrap_admin.sql`, website publik tetap dapat berjalan tetapi halaman `/admin` tidak akan memiliki tabel dan izin CMS yang lengkap.

Untuk workflow migration, jalankan file berikut secara berurutan:

```text
supabase/migrations/20260722000000_petbites_schema.sql
supabase/seed.sql
supabase/migrations/20260728000000_cms_ai_requests.sql
```

Seed pada `supabase/seed.sql` bersifat idempotent karena menggunakan `ON CONFLICT ... DO UPDATE`.
...
Website publik hanya menampilkan burung yang `is_active = true` dan konten yang `content_status = published`. Admin yang sedang login tetap dapat membaca draft di CMS, tetapi query dashboard publik juga memiliki filter eksplisit agar draft tidak ikut tampil atau masuk cache.
...
Untuk memverifikasi tabel CMS dan request pengguna, jalankan:
...
## 8. Mengubah Data Lewat CMS atau Table Editor

Cara yang disarankan adalah membuka `/admin` agar validasi form, role, status publikasi, dan perlindungan perubahan belum tersimpan tetap aktif. Table Editor dapat dipakai untuk pemeliharaan langsung oleh developer.
