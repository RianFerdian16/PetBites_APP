...
const translationFields: Record<string, string[]> = {
  birds: ["name_en", "description_en"],
  bird_foods: ["name_en", "benefits_en", "note_en"],
  toxic_entries: ["name_en", "explanation_en"],
  portion_rules: ["teaspoon_en", "morning_en", "evening_en"],
  recipes: ["title_en", "purpose_en", "ingredients_en", "steps_en"],
  bird_requests: [],
};
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const suggestion =
      action === "draft" || action === "improve" || action === "translate"
        ? sanitizeSuggestion(entity, action, parsed.suggestion)
        : null;
...
function buildPrompt(input: {
  ...
  const actionGuide: Record<string, string> = {
    ...
    translate:
      "Translate every Indonesian human-readable field into natural English. Return only the matching *_en fields in suggestion. Preserve list order exactly.",
    review:
  ...
  const outputContract =
    input.action === "draft" || input.action === "improve"
      ...
      : input.action === "translate"
        ? {
            suggestion: `object containing only these keys: ${translationFields[input.entity].join(", ")}`,
            text: "a short Indonesian reminder that the English draft still needs human review",
          }
        : {
            suggestion: null,
            text: "a clear Indonesian review with actionable points",
          };
  ...
  return JSON.stringify({
    ...
    strict_rules: [
      `For suggestions, never use fields outside: ${
        input.action === "translate"
          ? translationFields[input.entity].join(", ")
          : fields.join(", ")
      }.`,
...
function sanitizeSuggestion(entity: string, action: string, value: unknown) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;

  const allowed = new Set(
    action === "translate" ? translationFields[entity] : editableFields[entity],
  );
