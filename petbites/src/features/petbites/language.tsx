...
import { formatLocalizedNumber, localizeContent } from "./content-localization";
...
export function LanguageProvider({ children }: { children: ReactNode }) {
  ...
  const birdDescription = useCallback(
    (birdId: string, fallback: string) =>
      language === "en"
        ? (englishBirdDescriptions[birdId] ?? localizeContent(fallback, language))
        : fallback,
    [language],
  );
  ...
  const contentText = useCallback((value: string) => localizeContent(value, language), [language]);
